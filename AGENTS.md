...
- `.Codex\skills\design-loop\` — the design protocol. It loads for any visual work; its `references\design-law.md` binds every design decision.
...
- Dev server: `node C:\froni\.Codex\skills\design-loop\scripts\serve.mjs` (serves `site\` on http://localhost:4949, background it)
- Wait for Ferdinand's confirm: `node C:\froni\.Codex\skills\design-loop\scripts\wait.mjs` (rerun on NO_FEEDBACK_YET)
