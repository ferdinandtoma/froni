...
(function renderRev10(global) {
  const C = global.REV10_CONFIG;
  const D = global.REV10_DIAGRAMS;
  const A = global.REV10_ASSETS || { manifest: {}, data: {} };

  if (!C || !D) {
    throw new Error("Revision 10 source modules did not load");
  }
    ...
    return `
      <section class="page page-${page.number}" data-page-number="${page.number}" data-page-type="${page.type}"${constructionOnly ? ' data-construction-only="true"' : ""}>
        ...
        <footer class="page-footer">
          <span>Froni FRN-001 | Revision 10 | Factory development and quotation</span>
  ...
  global.__rev10Audit = function rev10Audit() {
  ...
  Promise.resolve(document.fonts ? document.fonts.ready : null).then(() => {
    global.__REV10_RENDER_DONE = true;
  });
})(window);
