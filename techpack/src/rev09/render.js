...
(function renderRev09(global) {
  const C = global.REV09_CONFIG;
  const D = global.REV09_DIAGRAMS;
  const A = global.REV09_ASSETS || { manifest: {}, data: {} };

  if (!C || !D) {
    throw new Error("Revision 09 source modules did not load");
  }
    ...
    return `
      <section class="page page-${page.number}" data-page-number="${page.number}" data-page-type="${page.type}"${constructionOnly ? ' data-construction-only="true"' : ""}>
        ...
        <footer class="page-footer">
          <span>Froni FRN-001 | Revision 09 | Factory development and quotation</span>
  ...
  global.__rev09Audit = function rev09Audit() {
    ...
    document.querySelectorAll("[data-construction-only='true']").forEach((page) => {
      if (page.dataset.pageNumber === "5" && page.querySelector("img, image")) {
        contentFailures.push("page 5 contains a raster in a code-drawn construction view");
      }
    });
  ...
  Promise.resolve(document.fonts ? document.fonts.ready : null).then(() => {
    global.__REV09_RENDER_DONE = true;
  });
})(window);
