...
(function renderRev12(global) {
  const C = global.REV12_CONFIG;
  const D = global.REV11_DIAGRAMS;
  const A = global.REV12_ASSETS || { manifest: {}, data: {} };

  if (!C || !D) {
    throw new Error("Revision 12 source modules did not load");
  }
  ...
  const assetFilename = (key) => (A.manifest[key]?.embeddedPath || "").split(/[\\/]/).pop();
    ...
    return `
      <section class="page page-${page.number}" data-page-number="${page.number}" data-page-type="${page.type}"${constructionOnly ? ' data-construction-only="true"' : ""}>
        ...
        <footer class="page-footer">
          <span>Froni FRN-001 | Revision ${esc(C.meta.revision)} | Factory development and quotation</span>
  ...
  const renderControl = () => `
    <div class="control-grid">
      ...
      <section class="status-panel">
        ...
        <p style="margin-top:3mm">This pack is the basis for the acceptance decision and for quoting development, sampling, embroidery setup, lead times, and a provisional bulk unit price.</p>
    ...
    <div class="three-col" style="height:100%">
      ...
      <div class="stack">
        ...
        <section class="card soft">
          ...
          <figure class="tape-preview" data-asset-key="careTapePreview">
            <img src="${A.data.careTapePreview}" alt="Printed care tape artwork preview">
            <figcaption>${esc(assetFilename("careTapePreview"))}</figcaption>
          </figure>
          <p class="tape-authority" data-authority-document="careTapeArtwork">Tape artwork draft supplied as Froni_Edition_One_Care_Tape_Draft_Rev01.svg; final width and fold set at pre-production.</p>
        </section>
        <section class="card accent">
  ...
  const approvedFlat = (attributes, src, caption, callouts) => `
    <figure ${attributes}>
      <div class="approved-flat-stage">
        <img class="approved-flat-image" src="${src}" alt="${esc(caption)}">
        <svg class="approved-flat-overlay" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
          ${callouts}
        </svg>
      </div>
      <figcaption class="flat-caption">${esc(caption)}</figcaption>
    </figure>`;

  const hoodAssemblyFigure = (key, view, header, imageAlt, overlay, caption) => `
    <figure class="diagram-panel hood-assembly-panel" data-audit-box="${esc(view)}" data-asset-key="${esc(key)}">
      <h2>${esc(header)}</h2>
      <div class="hood-assembly-stage" data-view="${esc(view)}">
        <img class="hood-assembly-image" src="${A.data[key]}" alt="${esc(imageAlt)}">
        <svg class="hood-assembly-overlay" viewBox="0 0 1376 768" preserveAspectRatio="xMidYMid meet" aria-label="${esc(imageAlt)}">
          ${overlay}
        </svg>
      </div>
      <figcaption class="hood-assembly-caption">${esc(caption)}</figcaption>
    </figure>`;
  ...
  const renderFrontBackFlats = () => `
    <div style="display:grid;grid-template-rows:minmax(0,1fr) auto;height:100%;min-height:0">
      <div class="technical-layout two">
        ${approvedFlat(
          'class="flat-panel" data-audit-box="front-flat" data-asset-key="flatFront" data-approved-flat="true" data-view="front" data-panel="front-body" data-construction="set-in" data-sleeve-length="full" data-body-length-cm="70" data-half-chest-cm="59" data-sleeve-cm="58" data-shoulder-cm="19" data-hem-rib-cm="6" data-cuff-rib-cm="6"',
          A.data.flatFront,
          "Front view",
          `<g data-callout-code="C1" data-location="cuff"><path class="flat-callout-line" d="M7 78 L18 88"/><circle class="flat-callout-marker" cx="7" cy="78" r="4.2"/><text class="flat-callout-text" x="7" y="79.2">C1</text></g>
           <g data-callout-code="C2" data-location="hem"><path class="flat-callout-line" d="M93 82 L62 88"/><circle class="flat-callout-marker" cx="93" cy="82" r="4.2"/><text class="flat-callout-text" x="93" y="83.2">C2</text></g>`,
        )}
        ${approvedFlat(
          'class="flat-panel" data-audit-box="back-flat" data-asset-key="flatBack" data-approved-flat="true" data-view="back" data-panel="back-body" data-construction="set-in" data-sleeve-length="full" data-body-length-cm="70" data-half-chest-cm="59" data-sleeve-cm="58" data-shoulder-cm="19" data-hem-rib-cm="6" data-cuff-rib-cm="6"',
          A.data.flatBack,
          "Back view",
          `<g data-callout-code="C3" data-location="hood-center-gusset"><path class="flat-callout-line" d="M93 16 L52 18"/><circle class="flat-callout-marker" cx="93" cy="16" r="4.2"/><text class="flat-callout-text" x="93" y="17.2">C3</text></g>`,
        )}
      </div>
  ...
  const renderViewFlats = () => `
    <div style="display:grid;grid-template-rows:minmax(0,1fr) auto;height:100%;min-height:0">
      <div class="technical-layout three">
        ${approvedFlat(
          'class="flat-panel" data-audit-box="side-flat" data-asset-key="flatSide" data-approved-flat="true" data-view="side" data-construction="set-in" data-sleeve-length="full" data-body-length-cm="70" data-half-chest-cm="59" data-sleeve-cm="58" data-shoulder-cm="19" data-hem-rib-cm="6" data-cuff-rib-cm="6"',
          A.data.flatSide,
          "Side view",
          `<g data-callout-code="C1" data-location="cuff"><path class="flat-callout-line" d="M7 87 L47 92"/><circle class="flat-callout-marker" cx="7" cy="87" r="4.2"/><text class="flat-callout-text" x="7" y="88.2">C1</text></g>
           <g data-callout-code="C2" data-location="hem"><path class="flat-callout-line" d="M93 78 L70 86"/><circle class="flat-callout-marker" cx="93" cy="78" r="4.2"/><text class="flat-callout-text" x="93" y="79.2">C2</text></g>`,
        )}
        ${approvedFlat(
          'class="flat-panel hood-pair" data-audit-box="hood-up-flat" data-asset-key="flatHoodUp" data-approved-flat="true" data-view="hood-up"',
          A.data.flatHoodUp,
          "Hood-up view",
          `<g data-callout-code="C4" data-location="hood-center-gusset"><path class="flat-callout-line" d="M93 15 L53 28"/><circle class="flat-callout-marker" cx="93" cy="15" r="4.2"/><text class="flat-callout-text" x="93" y="16.2">C4</text></g>`,
        )}
        ${approvedFlat(
          'class="flat-panel hood-pair" data-audit-box="hood-down-flat" data-asset-key="flatHoodDown" data-approved-flat="true" data-view="hood-down"',
          A.data.flatHoodDown,
          "Hood-down view",
          `<g data-callout-code="C5" data-location="hood-attachment"><path class="flat-callout-line" d="M93 47 L73 31"/><circle class="flat-callout-marker" cx="93" cy="47" r="4.2"/><text class="flat-callout-text" x="93" y="48.2">C5</text></g>`,
        )}
      </div>
      <div class="flat-footer">
        ${constructionLegend()}
  ...
  const renderHoodAssembly = () => `
    <div style="display:grid;grid-template-rows:minmax(0,1fr) auto;height:100%;min-height:0">
      <div class="technical-layout two">
        ${hoodAssemblyFigure(
          "hoodExplodedIllustration",
          "hood-exploded",
          "Exploded three-panel hood",
          "Exploded three-panel hood drawing",
          `<defs>
            <marker id="hoodExplodedArrow" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto">
              <path d="M0 0 L10 5 L0 10 Z" class="hood-arrow-fill"/>
            </marker>
          </defs>
          <g data-hood-label="left-side-panel" data-panel="hood-side-left">
            <path class="hood-label-leader" d="M176 714 L210 650"/>
            <text class="hood-label-text" x="28" y="742">Left side panel</text>
          </g>
          <g data-hood-label="center-gusset" data-panel="hood-center-gusset">
            <path class="hood-label-leader" d="M688 712 L688 668"/>
            <text class="hood-label-text" x="688" y="742" text-anchor="middle">Center gusset</text>
          </g>
          <g data-hood-label="right-side-panel" data-panel="hood-side-right">
            <path class="hood-label-leader" d="M1200 714 L1166 650"/>
            <text class="hood-label-text" x="1348" y="742" text-anchor="end">Right side panel</text>
          </g>
          <path class="hood-assembly-arrow" data-assembly-arrow="left-to-gusset" d="M548 385 L608 385" marker-end="url(#hoodExplodedArrow)"/>
          <path class="hood-assembly-arrow" data-assembly-arrow="right-to-gusset" d="M828 385 L768 385" marker-end="url(#hoodExplodedArrow)"/>`,
          "Outer and inner geometry, control points, and notches are derived from the approved pattern",
        )}
        ${hoodAssemblyFigure(
          "hoodCrossSectionIllustration",
          "hood-cross-section",
          "Double-layer assembly cross-section",
          "Horizontal double-layer hood assembly cross-section drawing",
          `<g data-hood-label="outer-self-fabric">
            <path class="hood-label-leader" d="M330 110 L402 245"/>
            <text class="hood-label-text" x="54" y="102">Outer self-fabric layer</text>
          </g>
          <g data-hood-label="embroidery-back">
            <path class="hood-label-leader" d="M520 548 L438 493"/>
            <text class="hood-label-text" x="530" y="575">Embroidery back concealed inside assembly</text>
          </g>
          <g data-hood-label="inner-self-fabric">
            <path class="hood-label-leader" d="M1100 666 L1002 532"/>
            <text class="hood-label-text" x="1322" y="700" text-anchor="end">Inner self-fabric layer</text>
          </g>`,
      ...
      <div class="pom-layout">
        <div class="stack">
          ...
          <section class="card soft">
            ...
            <p><strong>Method key:</strong> ${C.pom.new.slice(0, 6).map((row, index) => `N${index + 1} ${esc(row.name)} (${esc(row.disposition)})`).join("; ")}. No target values are assigned.</p>
    ...
    return `
      <div class="pom-layout hood">
        ...
        <div class="stack">
          ...
          ${table(["Key", "Pattern-defining point of measurement", "Measurement method", "Value disposition"], rows, "new-pom micro")}
          <section class="card accent">
  ...
  const renderBom = () => `
    <div class="stack" style="height:100%">
      ...
      <section class="card soft">
        <p style="margin:0">The polybag and cartons on this page are protective materials for factory shipment only. No other packaging is part of this bill of materials.</p>
      </section>
    </div>`;
        ...
        <div class="raster-column">
          ...
          <div class="artwork-lower">
            <div class="artwork-top">
              ...
              <section class="back-acceptance-panel" data-authority-document="backAcceptanceSheet">
                <div class="back-acceptance-heading">
                  <strong>FRN-001-E artwork sheet, Rev 01</strong>
                  <span>Back face acceptance</span>
                </div>
                <div class="back-acceptance-grid">
                  <figure class="back-acceptance-figure back-acceptance-pin" data-asset-key="approvedAsymmetryMap">
                    <img src="${A.data.approvedAsymmetryMap}" alt="Approved four-point back-icon asymmetry pin map">
                    <figcaption>Four acceptance pins</figcaption>
                  </figure>
                  <div class="back-mirror-pair">
                    <figure class="back-acceptance-figure" data-asset-key="mirrorCompositeBlessing">
                      <img src="${A.data.mirrorCompositeBlessing}" alt="Left doubled, the blessing side">
                      <figcaption>Left doubled, the blessing side</figcaption>
                    </figure>
                    <figure class="back-acceptance-figure" data-asset-key="mirrorCompositeJudge">
                      <img src="${A.data.mirrorCompositeJudge}" alt="Right doubled, the judge side">
                      <figcaption>Right doubled, the judge side</figcaption>
                    </figure>
                  </div>
                </div>
                <p class="back-acceptance-file">${esc((A.manifest.backAcceptanceSheet?.embeddedPath || "").split(/[\\/]/).pop())}</p>
              </section>
            </div>
      ...
      <div class="stack">
        <section class="card accent">
          ...
          <div class="hood-reference-grid" data-asset-key="hoodExteriorReference">
            ...
            <div class="hood-reference-assets">
              <div>
                ...
                <p class="hood-reference-file"><strong>Reference:</strong> ${esc(assetFilename("hoodExteriorReference"))}</p>
              </div>
              <figure class="hood-lettering-preview hood-lettering-preview-tall" data-asset-key="hoodExteriorLetteringPreview" data-authority-document="hoodExteriorLettering">
                <img src="${A.data.hoodExteriorLetteringPreview}" alt="Exterior hood letterform artwork preview">
                <figcaption>Artwork preview</figcaption>
              </figure>
            </div>
          </div>
          <p class="hood-preview-file">${esc(assetFilename("hoodExteriorLetteringPreview"))}</p>
        ...
        <section class="card">
          ...
          <div class="hood-reference-grid hood-reference-grid-interior" data-asset-key="hoodInteriorReference">
            ...
            <div>
              ...
              <p class="hood-reference-file"><strong>Reference:</strong> ${esc(assetFilename("hoodInteriorReference"))}</p>
              <figure class="hood-lettering-preview hood-lettering-preview-wide" data-asset-key="hoodInteriorLetteringPreview" data-authority-document="hoodInteriorLettering">
                <img src="${A.data.hoodInteriorLetteringPreview}" alt="Interior hood verse artwork preview">
                <figcaption>${esc(assetFilename("hoodInteriorLetteringPreview"))}</figcaption>
              </figure>
            </div>
          </div>
  ...
  global.__rev12Audit = function rev12Audit() {
    ...
    if (footers.length !== C.pages.length) contentFailures.push(`expected ${C.pages.length} footers, found ${footers.length}`);
    footers.forEach((footer, index) => {
      if (!footer.textContent.includes(`Revision ${C.meta.revision}`)) {
        contentFailures.push(`page ${index + 1} footer does not identify Revision ${C.meta.revision}`);
      }
    });
    ...
    const foundLocations = new Set([
      ...[...document.querySelectorAll("[data-location]")].map((element) => element.dataset.location),
      ...[...document.querySelectorAll("[data-visible-location]")].map((element) => element.dataset.visibleLocation),
    ]);
    ...
    const pageFive = document.querySelector("[data-page-number='5']");
    if (!pageFive || pageFive.querySelectorAll(".hood-assembly-image").length !== 2) {
      contentFailures.push("page 5 does not contain exactly two approved hood illustrations");
    }
    if (!pageFive || pageFive.querySelectorAll("[data-hood-label]").length !== 6) {
      contentFailures.push("page 5 does not contain exactly six hood illustration labels");
    }
    if (!pageFive || pageFive.querySelectorAll("[data-assembly-arrow]").length !== 2) {
      contentFailures.push("page 5 does not contain exactly two assembly arrows");
    }
    if (document.querySelectorAll("[data-approved-flat='true']").length !== 5) {
      contentFailures.push("approved construction-flat image count is not five");
    }

    const requiredMissing = ["frontProductionVector", "digitizedMasters"];
    requiredMissing.forEach((key) => {
      if (!document.querySelector(`[data-missing-asset='${key}']`)) markerFailures.push(`missing production-artwork placeholder: ${key}`);
    });

    const pageTen = document.querySelector("[data-page-number='10']");
    if (!pageTen || !pageTen.querySelector("[data-authority-document='backAcceptanceSheet']")) {
      contentFailures.push("page 10 does not identify the approved back acceptance sheet");
    }
    for (const key of ["approvedAsymmetryMap", "mirrorCompositeBlessing", "mirrorCompositeJudge"]) {
      if (!pageTen || !pageTen.querySelector(`[data-asset-key='${key}'] img`)) {
        contentFailures.push(`page 10 does not render the approved back acceptance asset: ${key}`);
      }
    }
    const pageTwo = document.querySelector("[data-page-number='2']");
    if (!pageTwo || !pageTwo.querySelector("[data-asset-key='careTapePreview'] img")) {
      contentFailures.push("page 2 does not render the care tape artwork preview");
    }
    const pageEleven = document.querySelector("[data-page-number='11']");
    for (const key of ["hoodExteriorLetteringPreview", "hoodInteriorLetteringPreview"]) {
      if (!pageEleven || !pageEleven.querySelector(`[data-asset-key='${key}'] img`)) {
        contentFailures.push(`page 11 does not render the lettering preview: ${key}`);
      }
    }
    ...
    const requiredText = [
      ...
      "Bulk cutting may begin only after all six listed approvals are recorded.",
      "Ο Ω Ν",
    ];
    ...
    if (bodyText.includes(String.fromCodePoint(0x2013))) contentFailures.push("document contains an en dash");
  ...
  Promise.resolve(document.fonts ? document.fonts.ready : null).then(() => {
    global.__REV12_RENDER_DONE = true;
  });
})(window);
